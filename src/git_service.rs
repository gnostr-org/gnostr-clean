...
pub fn start_git() {
    ...
    match (
        ...
        (_, _, _, _, _, Some(commit_from), Some(commit_to), _) => {
            if let Err(e) =
                handle_commit_range(repo, Some(commit_from.clone()), Some(commit_to.clone()))
            {
                eprintln!("Application error: {}", e);
                process::exit(0);
            }
...
// 扫描commit，根据时间
fn handle_commit_range_by_time(
    repo: Repository,
    since: &str,
    until: &str,
) -> Result<(), git2::Error> {
...
pub fn handle_branches_by_name(repo: Repository, branch_name: &str) -> Result<(), Error> {
